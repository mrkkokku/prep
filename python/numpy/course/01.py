...
# 거의 들지않는 수준으로 처리가 가능하다고 


# stride 추가 설명
# 넘파이어레이는 메모리에 순차적으로 모든데이터가 저장 되고
# stirde에 따라 그 배치가 출력시점에 결정되어 행렬처럼 보이게 됨
# stride는 데이터의 출력시점에 몇 바이트를 띄고 표기하나의 문제임